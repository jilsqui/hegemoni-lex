  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans selection:bg-black selection:text-white overflow-x-hidden">
      ...
      <header className="pt-20 md:pt-28">
        ...
        {featuredArticle ? (
          <div className="max-w-7xl mx-auto px-4 md:px-6 pt-6 md:pt-10 pb-8 md:pb-12">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
              
              {/* LEFT: Featured Image */}
              <div className="lg:col-span-7 relative min-h-[280px] md:min-h-[380px] lg:min-h-[500px] bg-gray-100 overflow-hidden hover-elegant">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10 z-10">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="bg-white text-black px-3 py-1 text-[9px] font-bold uppercase tracking-[0.15em]">
                  ...
                  <Link href={`/artikel/${featuredArticle.slug}`} className="group">
                    <h1 className="text-2xl md:text-4xl lg:text-[2.75rem] font-serif font-bold text-white leading-[1.15] mb-4 group-hover:opacity-80 transition-opacity duration-300">
                      {featuredArticle.title}
                    </h1>
                  </Link>
                  <p className="text-white/70 text-sm md:text-base leading-relaxed max-w-xl line-clamp-2 font-light hidden md:block">
                    {featuredArticle.excerpt || featuredArticle.content.substring(0, 160) + "..."}
                  </p>
                  <div className="flex items-center gap-3 mt-5">
                    <div className="w-7 h-7 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white text-[10px] font-bold border border-white/10">
                ...
                <div className="absolute top-5 left-5 z-10">
                  <span className="text-[9px] uppercase tracking-[0.2em] font-bold text-white/60 bg-black/40 backdrop-blur-sm px-3 py-1.5">Fokus Utama</span>
                </div>
              </div>
              ...
              <div className="lg:col-span-5 bg-white flex flex-col lg:border-l border-t lg:border-t-0 border-gray-200">
                <div className="hidden lg:block px-6 pt-5 pb-3">
                  <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400">Terbaru</span>
                </div>
                {trendingArticles.length > 0 ? trendingArticles.map((article, index) => (
                  <Link 
                    key={article.id} 
                    href={`/artikel/${article.slug}`}
                    className="flex items-start gap-4 px-5 lg:px-6 py-4 hover:bg-gray-50/80 transition-all duration-300 group/item flex-1 border-b border-gray-100 last:border-b-0"
                  >
                    <span className="text-3xl font-serif font-bold text-gray-100 group-hover/item:text-gray-300 transition-colors duration-300 leading-none mt-0.5 w-10 flex-shrink-0 text-right">
                      {String(index + 1).padStart(2, '0')}
                    </span>
                    <div className="flex-1 min-w-0">
                      <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-gray-400 mb-1.5 block">
                        {article.category}
                      </span>
                      <h3 className="text-sm font-serif font-bold leading-snug line-clamp-2 group-hover/item:text-gray-600 transition-colors duration-300">
                        {article.title}
                      </h3>
                    ...
                    {article.image && (
                      <div className="w-16 h-16 bg-gray-100 flex-shrink-0 overflow-hidden hover-elegant">
        ...
        <div className="bg-gray-950 text-white py-3 overflow-hidden">
          <div className="animate-marquee whitespace-nowrap">
            {["LEGISLASI", "OPINI", "HUKUM PERDATA", "HUKUM PIDANA", "BISNIS", "KETENAGAKERJAAN", "HAK ASASI MANUSIA", "LEGISLASI", "OPINI", "HUKUM PERDATA", "HUKUM PIDANA", "BISNIS", "KETENAGAKERJAAN", "HAK ASASI MANUSIA"].map((cat, i) => (
              <Link 
                key={i} 
                href={`/artikel?q=${cat}`} 
                className="inline-block text-[10px] font-bold uppercase tracking-[0.2em] mx-6 hover:text-gray-400 transition-colors duration-300"
              >
                {cat} <span className="text-gray-700 mx-2">·</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* ================================================================ */}
      <div className="bg-gray-50 text-black py-12 px-6 border-b border-gray-200 flex justify-center items-center group cursor-help transition-all duration-500 hover:bg-white">
        <div className="relative h-16 w-full max-w-4xl flex items-center justify-center overflow-hidden">
            <h2 className="absolute transition-all duration-500 ease-in-out opacity-100 group-hover:opacity-0 group-hover:blur-md group-hover:-translate-y-4 font-serif text-xl md:text-2xl italic tracking-wide z-10 text-center text-gray-600">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 pb-4 gap-4">
              <div>
                <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-2 block">Artikel</span>
                <h2 className="text-3xl md:text-4xl font-serif font-medium leading-none text-gray-900">Publikasi Terbaru</h2>
              </div>
              <Link href="/artikel" className="text-[10px] font-bold hover:text-black uppercase tracking-[0.15em] text-gray-400 transition-colors duration-300 border-b border-transparent hover:border-black pb-0.5">
                LIHAT SEMUA →
              </Link>
            </div>
            ...
            {latestArticles.length === 0 ? (
              ...
              <div className="space-y-0">
                ...
                {latestArticles[0] && (
                  <ScrollPopOut>
                  <Link href={`/artikel/${latestArticles[0].slug}`} className="group block pb-10 mb-10 border-b border-gray-200">
                    <article>
                      {latestArticles[0].image && (
                        <div className="aspect-[16/9] w-full bg-gray-100 mb-6 overflow-hidden hover-elegant">
                          <img 
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-[9px] font-bold px-2.5 py-1 uppercase tracking-[0.15em] bg-gray-900 text-white">
                      ...
                      <h3 className="text-2xl md:text-3xl font-serif font-bold mb-3 leading-[1.2] text-gray-900 group-hover:text-gray-600 transition-colors duration-300">
                        {latestArticles[0].title}
                      </h3>
                      <p className="text-gray-500 text-sm leading-[1.8] line-clamp-3 font-light max-w-2xl">
                        {latestArticles[0].excerpt || latestArticles[0].content.substring(0, 200) + "..."}
                      </p>
                      <div className="flex items-center gap-2 mt-4">
                        <span className="text-[10px] text-gray-400 font-light">oleh</span>
                        <span className="text-[10px] font-medium text-gray-700">{latestArticles[0].author.name || 'Redaksi'}</span>
                      </div>
                    </article>
                  </Link>
                  </ScrollPopOut>
                ...
                {latestArticles.length > 2 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-10 mb-10 border-b border-gray-200">
                    {latestArticles.slice(1, 3).map((article) => (
                      ...
                      <Link href={`/artikel/${article.slug}`} className="group block">
                        <article>
                          ...
                          <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-gray-400 block mb-2">
                            {article.category}
                          </span>
                          <h3 className="text-lg font-serif font-bold leading-snug line-clamp-2 mb-2 text-gray-900 group-hover:text-gray-600 transition-colors duration-300">
                            {article.title}
                          </h3>
                          <p className="text-gray-400 text-xs leading-relaxed line-clamp-2 font-light mb-2.5 hidden md:block">
                ...
                {latestArticles.slice(3).map((article) => (
                  <ScrollPopOut key={article.id}>
                  <Link href={`/artikel/${article.slug}`} className="group flex gap-5 py-6 items-start border-b border-gray-100 last:border-b-0">
                    {article.image && (
                      <div className="w-28 h-20 md:w-36 md:h-24 bg-gray-100 flex-shrink-0 overflow-hidden hover-elegant">
                        <img 
                          src={article.image} 
                          alt={article.title} 
                          className="w-full h-full object-cover"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="text-base md:text-lg font-serif font-bold leading-snug line-clamp-2 mb-1.5 text-gray-900 group-hover:text-gray-600 transition-colors duration-300">
                        {article.title}
                      </h3>
          ...
          <aside className="lg:col-span-4">
            ...
            {/* POPULER SECTION */}
            <div className="mb-14 sticky top-28">
              <div className="border-b border-gray-900 pb-3 mb-6">
                <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-900">Populer</h3>
              </div>
              ...
              {popularArticles.length > 0 ? (
                <div className="space-y-0">
                  {popularArticles.map((article, index) => (
                    ...
                    >
                      <span className="text-2xl font-serif font-bold text-gray-200 group-hover:text-gray-400 transition-colors duration-300 leading-none mt-0.5 w-8 flex-shrink-0 text-right">
                        {String(index + 1).padStart(2, '0')}
                      </span>
                      <div className="flex-1 min-w-0">
                        <span className="text-[8px] font-bold uppercase tracking-[0.15em] text-gray-400 block mb-1.5">
                          {article.category}
                        </span>
                        <h4 className="text-sm font-serif font-bold leading-snug line-clamp-2 text-gray-900 group-hover:text-gray-600 transition-colors duration-300">
                          {article.title}
                        </h4>
              ...
              <div className="mt-14">
                <div className="border-b border-gray-900 pb-3 mb-6">
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-900">Kategori</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {["LEGISLASI", "OPINI", "HUKUM PERDATA", "HUKUM PIDANA", "BISNIS", "KETENAGAKERJAAN", "HAK ASASI MANUSIA"].map((cat) => (
                    <Link 
                      key={cat}
                      href={`/artikel?q=${cat}`}
                      className="text-[9px] font-bold uppercase tracking-[0.1em] border border-gray-200 px-3.5 py-2 hover:bg-gray-900 hover:text-white hover:border-gray-900 transition-all duration-300"
                    >
                      {cat}
                    </Link>
                  ))}
              ...
              {/* TENTANG KAMI MINI */}
              <div className="mt-14 bg-gray-50 p-6 border-l-2 border-gray-900">
                <div className="flex items-center gap-3 mb-4">
      ...
      {/* 4. FOOTER                                                        */}
      {/* ================================================================ */}
      <footer className="bg-gray-950 text-white py-16 md:py-24 px-4 md:px-6 font-sans">
        ...
        <div className="max-w-7xl mx-auto mt-16 md:mt-24 pt-8 border-t border-gray-800/50 flex flex-col md:flex-row justify-between items-center text-[10px] text-gray-600 font-mono uppercase tracking-widest gap-4">
